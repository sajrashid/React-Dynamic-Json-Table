...
const Thead = ({ state, dispatch }) => {
    ...
    const createHeader = () => {
        return columns.map((key) => {
            ...
            if (isLabel) {
                return <th className={cssClasses} id={key} key={key}
                    onClick={(e) => dispatch({ type: ACTIONS.SORT, payload: { id: e.currentTarget.id } })}
                    dangerouslySetInnerHTML={createMarkup(isLabel[key])}></th>
            }
            if (isIcon) {
                return <th className={cssClasses} id={key} key={key}
                    onClick={(e) => dispatch({ type: ACTIONS.SORT, payload: { id: e.currentTarget.id } })}
                    dangerouslySetInnerHTML={createMarkupLiteral(key, isIcon[key], key)}></th>
            }
