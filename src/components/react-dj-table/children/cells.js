...
const Cells = ({ state, dispatch, row, editable }) => {
    ...
    const createCells = (row) => {
        return columns.map((key) => {
            ...
            const isDate = dateColArr.find(function (b) { return b.hasOwnProperty(key) })
            ...
            if (editable) {
               ...
               if (isDate) return <td className={editcssClasses} key={key}> <input type="text" name={key} onChange={onChange} value={row[key].toString()}></input> </td>
